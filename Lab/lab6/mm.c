...
/* access kernel pgd for copying higher-half entries */
...
static inline void* page_to_addr(struct page* p) {
    ...
    unsigned long pa = buddy_memory_base + index * PAGE_SIZE;
...
static inline struct page* addr_to_page(void* addr) {
    ...
    // 如果傳進來的是高位虛擬位址，先轉回物理位址
    if (va >= PAGE_OFFSET) {
        pa = va - PAGE_OFFSET;
    }
    
    unsigned long offset = pa - buddy_memory_base;
...
void free_pages(struct page* p) {
    ...
    // CoW pages may be shared by multiple tasks. Only return to buddy system at refcount zero.
    p->refcount--;
    if (p->refcount > 0) {
        return;
    }
...
// =============================== Lab6: Basic Exercise 2 - Virtual Memory in User Space  ===============================
...
#define PTE_INDEX(va) (((unsigned long)(va) >> PTE_SHIFT) & 0x1FF)
...
unsigned long *alloc_user_pgd(void) {
    unsigned long *new_pgd = (unsigned long *)allocate(4096); // 一個 pgd table 是 4096 大小
    if (!new_pgd) return NULL;
    memset(new_pgd, 0, 4096);

    /* Copy kernel higher-half PGD entries into the new PGD.
     * Use Sv39 PGD index calculation: PGD_SHIFT = 30, mask 0x1FF.
     * Copy a small range (4 entries) that the kernel uses for linear/higher-half mapping.
     */
    // 取出 virtual address 的第 38 到 30 位，這 9 個 bit 就是 Sv39 的 PGD index
    unsigned long kernel_index = (PAGE_OFFSET >> 30) & 0x1FF;
    for (int i = 0; i < 4; i++) {
        new_pgd[kernel_index + i] = pgd[kernel_index + i];
    }
...
// 幫忙鋪路
// 給 pgd 然後要把 va map 到 pa
static void pagewalk(unsigned long *root_pgd, unsigned long va, unsigned long pa, unsigned long prot) {
    // Level 2 PGD -> 找 PMD Table
    unsigned long *pgd_entry = &root_pgd[PGD_INDEX(va)];

    if (!PTE_VALID(*pgd_entry)) {
        // 分配一個新的 PMD table
        unsigned long *new_pmd_va = (unsigned long *)allocate(4096);
        memset(new_pmd_va, 0, 4096);
        // 將分配到的 VA 轉回 PA
        unsigned long new_pmd_pa = (unsigned long)new_pmd_va - PAGE_OFFSET;
        // 將這個新的 PMD PA 寫進 PGD entry 這裡只設 PTE_V，因為它是指向下一層表的指標，不是指向資料
        *pgd_entry = MAKE_PTE(new_pmd_pa, PTE_V);
    }

    unsigned long pmd_pa = ((*pgd_entry) >> 10) << 12;
    unsigned long *pmd_table = (unsigned long *)(pmd_pa + PAGE_OFFSET);

    // Level 1 PMD -> 找 PTE 表格
    unsigned long *pmd_entry = &pmd_table[PMD_INDEX(va)];
    
    if (!PTE_VALID(*pmd_entry)) {
        // 分配一個新的 PTE table
        unsigned long *new_pte_va = (unsigned long *)allocate(4096);
        memset(new_pte_va, 0, 4096);
        // 將分配到的 VA 轉回 PA
        unsigned long new_pte_pa = (unsigned long)new_pte_va - PAGE_OFFSET;
        // 將這個新的 PTE PA 寫進 PMD entry
        *pmd_entry = MAKE_PTE(new_pte_pa, PTE_V);
    }

    unsigned long pte_pa = ((*pmd_entry) >> 10) << 12;
    unsigned long *pte_table = (unsigned long *)(pte_pa + PAGE_OFFSET);

    // Level 0 PTE -> PA
    unsigned long *pte_entry = &pte_table[PTE_INDEX(va)];
    *pte_entry = MAKE_PTE(pa, prot | PTE_V);
}

// 如果有一大塊 size 需要 map 到 pa，傳統上需要寫好多次 pagewalk，用 man_pages 可以幫你依次做完全部
// 就是一個迴圈 裡面跑 pagewalk
void map_pages(unsigned long *root_pgd, unsigned long va, unsigned long size, unsigned long pa, unsigned long prot) {
    // 1. 將起始位址向下對齊到 4KB (Page Boundary)
    // 這樣可以確保我們從完整的頁面開頭開始鋪路
    unsigned long start_va = va & ~(4096UL - 1);
    unsigned long start_pa = pa & ~(4096UL - 1);
    
    // 2. 計算結束位址，並向上對齊到 4KB
    // 假設 size 是 5000 (跨越了兩頁)，加上 4095 再向下對齊，就會精準涵蓋兩頁 (8192)
    unsigned long end_va = (va + size + 4096UL - 1) & ~(4096UL - 1);

    // 3. 迴圈工廠開始運作！一頁一頁往前推進
    unsigned long curr_va = start_va;
    unsigned long curr_pa = start_pa;

    while (curr_va < end_va) {
        pagewalk(root_pgd, curr_va, curr_pa, prot);
...
/* Unmap pages in [va, va+size). Does not free page tables. */
void unmap_pages(unsigned long *root_pgd, unsigned long va, unsigned long size) {
    unsigned long start_va = va & ~(4096UL - 1);
    unsigned long end_va = (va + size + 4096UL - 1) & ~(4096UL - 1);
    unsigned long curr_va = start_va;

    while (curr_va < end_va) {
        unsigned long *pgd_entry = &root_pgd[PGD_INDEX(curr_va)];
        if (!PTE_VALID(*pgd_entry)) {
            curr_va += 4096UL;
            continue;
        }
        unsigned long pmd_pa = ((*pgd_entry) >> 10) << 12;
        unsigned long *pmd_table = (unsigned long *)(pmd_pa + PAGE_OFFSET);
        unsigned long *pmd_entry = &pmd_table[PMD_INDEX(curr_va)];
        if (!PTE_VALID(*pmd_entry)) {
            curr_va += 4096UL;
            continue;
        }
        unsigned long pte_pa = ((*pmd_entry) >> 10) << 12;
        unsigned long *pte_table = (unsigned long *)(pte_pa + PAGE_OFFSET);
        unsigned long *pte_entry = &pte_table[PTE_INDEX(curr_va)];
        *pte_entry = 0; /* clear mapping */
        curr_va += 4096UL;
    }
}
